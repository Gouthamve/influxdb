...
// ShardLocation is used when calling NewShard to specify the shard's
...
func NewShard(id uint64, index *DatabaseIndex, location ShardLocation, options EngineOptions) *Shard {
	// Configure statistics collection.
	key := fmt.Sprintf("shard:%s:%d", location.Path, id)
...
func (s *Shard) Path() string { return s.location.Path }
...
func (s *Shard) Open() error {
	if err := func() error {
		...
		e, err := NewEngine(s.location.Path, s.location.WALPath, s.options)
		if err != nil {
...
func (s *Shard) DiskSize() (int64, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	stats, err := os.Stat(s.location.Path)
	if err != nil {
		return 0, err
	}
