...
func TestShardWriteAndIndex(t *testing.T) {
	...
	sh := tsdb.NewShard(1, index, tsdb.ShardLocation{Path: tmpShard, WALPath: tmpWal}, opts)
...
func benchmarkWritePoints(b *testing.B, mCnt, tkCnt, tvCnt, pntCnt int) {
	...
	for n := 0; n < b.N; n++ {
		...
		tmpShard := path.Join(tmpDir, "shard")
		tmpWal := path.Join(tmpDir, "wal")
		shard := tsdb.NewShard(1, index, tsdb.ShardLocation{Path: tmpShard, WALPath: tmpWal}, tsdb.NewEngineOptions())
		shard.Open()
