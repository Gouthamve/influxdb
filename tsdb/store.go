	...
	for db := range s.databaseIndexes {
		...
		for _, rp := range rps {
			...
			for _, sh := range shards {
				...
				shard := NewShard(shardID, s.databaseIndexes[db], sl, s.EngineOptions)
				err = shard.Open()
				if err != nil {
...
func (s *Store) CreateShard(database, retentionPolicy string, shardID uint64) error {
	...
	shard := NewShard(shardID, db, sl, s.EngineOptions)
...
func (s *Store) deleteShard(shardID uint64) error {
	...
	if err := os.RemoveAll(sh.location.Path); err != nil {
		return err
	}

	if err := os.RemoveAll(sh.location.WALPath); err != nil {
		return err
	}
...
func (s *Store) DeleteRetentionPolicy(database, name string) error {
	...
	// database.
	for shardID, sh := range s.shards {
		if sh.location.Database == database && sh.location.RetentionPolicy == name {
			// Delete the shard from disk.
			if err := s.deleteShard(shardID); err != nil {
				return err
			}
...
func (s *Store) DeleteMeasurement(database, name string) error {
	...
	for _, sh := range s.shards {
		if sh.location.Database != database {
			continue
		}
...
func (s *Store) BackupShard(id uint64, since time.Time, w io.Writer) error {
	...
	path, err := relativePath(s.path, shard.location.Path)
	if err != nil {
		return err
	}
...
func (s *Store) ShardRelativePath(id uint64) (string, error) {
	...
	return relativePath(s.path, shard.location.Path)
}
